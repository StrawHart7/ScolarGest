-- Catalogues système Togo (cycles, niveaux, séries) + plans d'abonnement.
-- Contenu identique à supabase/seed.sql, mais en migration versionnée : `supabase/seed.sql`
-- ne s'applique qu'avec `supabase db reset` (local), jamais avec `db push` (staging/prod).
-- Idempotent (on conflict) pour rester safe si déjà appliqué manuellement.
