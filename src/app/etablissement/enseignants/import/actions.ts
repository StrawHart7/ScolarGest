...
import { z } from 'zod';
import {
  preparerImportEnseignants,
  executerImportEnseignants,
  type ImportRapport,
...
export async function analyserFichierEnseignants(
  ...
  try {
    const { analyse } = await preparerImportEnseignants(envoi.buffer, envoi.anneeScolaireId);
...
export async function confirmerImportEnseignants(
  formData: FormData,
  ...
  try {
    const { analyse, rapport } = await executerImportEnseignants(
      envoi.buffer,
      envoi.anneeScolaireId,
    );
    revalidatePath('/etablissement/enseignants');
