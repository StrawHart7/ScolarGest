...
import {
  ...
  confirmerImportEnseignants,
  type ImportActionResult,
} from './actions';

/**
 * Import des enseignants en deux temps : analyser, relire le bilan, confirmer.
 *
...
export function ImportForm({ anneeScolaireId }: { anneeScolaireId: string }) {
  ...
  function analyser(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      let res: Awaited<ReturnType<typeof analyserFichierEnseignants>> | undefined;
      try {
        res = await analyserFichierEnseignants(formData);
      } catch {
        res = undefined;
      }
      if (!res) {
  ...
  function confirmer() {
    ...
    setMessage(null);
    startTransition(async () => {
      let res: Awaited<ReturnType<typeof confirmerImportEnseignants>> | undefined;
      try {
        res = await confirmerImportEnseignants(formData);
      } catch {
        res = undefined;
      }
      if (!res) {
