...
/** Nombre de lignes du tableau, vides comprises — le modèle en compte 20. */
const LIGNES_TABLEAU = 20;

export function renderBulletinSecondaireHtml(
  input: BulletinTemplateInput & { eleve: { sexe?: 'M' | 'F' } },
): string {
  const { etablissement, eleve, donnees, classeNom, anneeScolaireLibelle, periodeLabel } = input;
  ...
  const lignesVides = Array.from(
    { length: Math.max(0, LIGNES_TABLEAU - donnees.matieres.length) },
    () =>
      `<tr class="vide"><td class="c-matiere">&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>`,
  ).join('');
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  table.notes tr.vide td { height: 17px; }
